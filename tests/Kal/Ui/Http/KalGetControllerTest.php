...
function getKalAuthHeaders(): array
{
    return [
...
it('answers 200 with the organizer view including the invite token', function (): void {
    ...
    $client->request('GET', '/kal/'.$kal->id->value(), server: getKalAuthHeaders());
...
it('answers 404 when the kal does not exist', function (): void {
    ...
    $client->request('GET', '/kal/01J5M6XQBR4GTYHN8KZXP0F1W9', server: getKalAuthHeaders());
...
it('answers 400 when the id is not a valid ulid', function (): void {
    ...
    $client->request('GET', '/kal/not-a-ulid', server: getKalAuthHeaders());
