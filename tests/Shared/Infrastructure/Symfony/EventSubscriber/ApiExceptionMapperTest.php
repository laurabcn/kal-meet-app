...
it('maps ConflictException to 409 using the type, not the message', function (): void {
    $exception = KalAlreadyExistsException::create();
    $mapped = $this->mapper->map($exception);
    ...
    expect($exception)->toBeInstanceOf(ConflictException::class)
        ...
        ->and($mapped->message)->toBe('A kal with this id already exists.');
});
